...
bool broadcast_connect()
{
    ...
    while(true) {
        ...
        if (check_request_method(client_message, PERMISSION_REQUEST_METHOD)) {
            ...
            const char* sessionJSON = session_key_to_json(generate_session_key());
            puts(sessionJSON);
            tcp_send(info.conn_d, sessionJSON);
            //tcp_send(sock_tcp_a, "hello");
            //tcp_conn_send(sock_tcp_a, info.client_inf, session_key_to_json(generate_session_key()));
            //client_message = tcp_accept_read(sock_tcp_a).message;
            client_message = tcp_read(info.conn_d);
            if (strcmp(client_message, KEY_ACCEPTED) != 0) {
                puts("Client doesn't accept session key");
                close(sock_d);
                return false;
            }
            tcp_send(info.conn_d, RESPONSE_CONNECT_SUCCESS);
            //tcp_conn_send(sock_tcp_a, info.client_inf,RESPONSE_CONNECT_SUCCESS);
